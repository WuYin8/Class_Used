...
class Model
{
	...
	//count  max  min  sum
	function count($field = null , $table = null , $where = null)
	{
		if (empty($field)) {
			$field = $this->fields['PRI'];
		}
		$result = $this->field('count('.$field.') as count')->table($table)->where($where)->select();
		return $result[0]['count'];
	}
	
	function max($field = null , $table = null , $where = null)
	{
		if (empty($field)) {
			$field = $this->fields['PRI'];
		}
		$result = $this->field('max('.$field.') as max')->table($table)->where($where)->select();
		return $result[0]['max'];
	}
	
	function min($field = null , $table = null , $where = null)
	{
		if (empty($field)) {
			$field = $this->fields['PRI'];
		}
		$result = $this->field('min('.$field.') as min')->table($table)->where($where)->select();
		return $result[0]['min'];
	}
	
	function sum($field = null , $table = null , $where = null)
	{
		if (empty($field)) {
			$field = $this->fields['PRI'];
		}
		$result = $this->field('sum('.$field.') as sum')->table($table)->where($where)->select();
		return $result[0]['sum'];
	}
}
